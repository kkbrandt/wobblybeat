define('wobblyShape', function(require) {
    ...
    const teapot = require('json!teapot.json');
    ...
    class WobblyShape {
        ...
        */
        ...
        set shape(str) {
            ...
            switch (str) {
                case 'Cube':
                    ...
                    break;
                case 'Plane':
                    this.geometry = new THREE.PlaneGeometry(sz * 3, sz * 3, 100, 100);
                    sidedness = THREE.DoubleSide;
                    break;
                case 'Ring':
                    this.geometry = new THREE.RingGeometry(sz / 2, sz * 2, 1000);
                    sidedness = THREE.DoubleSide;
                    break;
                case 'Icosahedron':
                    ...
                    break;
                case 'Torus':
                    this.geometry = new THREE.TorusGeometry(sz, sz / 2, 90, 90);
                    break;
                case 'TorusKnot':
                    this.geometry = new THREE.TorusKnotGeometry(sz, sz / 5, 90, 90);
                    break;
                case 'Sphere':
                    // this.geometry = new THREE.SphereGeometry(sz, 38, 38);
                    this.geometry = new THREE.SphereGeometry(sz, 90, 90);
                    break;
                case 'Teapot':
                    prms = this.loadGeometryFromFile('lib/modules/teapot.json', sz);
                    sidedness = THREE.DoubleSide;
                    break;
                case 'Spinner':
                    prms = this.loadGeometryFromFile('lib/modules/FidgetSpinner.json', sz);
                    sidedness = THREE.DoubleSide;
                    break;
                case 'Trump':
                    prms = this.loadGeometryFromFile('lib/modules/trump1.json', sz);
                    sidedness = THREE.DoubleSide;
                    break;
            }
            return prms.then(() => {
        ...
        loadGeometryFromFile(filePath, scale) {
        ...
        moveVertices(values) {
            ...
            for (var i = 0, l = geometry.vertices.length; i < l; i++) {
                ...
                // this.geometry.colors[i] = this.colorizeFn(i / geometry.vertices.length);
